...
// LayoutEngineService şu an instance metodu olarak çalışıyor (statik değil),
// bu yüzden tek bir instance oluşturup onu kullanıyoruz.
...
export const NetworkExplorerPage: React.FC<NetworkExplorerPageProps> = ({
  ...
  // NOT: calculateLayout şu an CIRCULAR/GRID/FORCE ayrımını uygulamıyor,
  // node/edge'leri olduğu gibi döndürüyor. Layout algoritması sonradan
  // LayoutEngineService içine eklenmeli.
  ...
  const handleExport = (format: ExportFormat) => {
    ...
    // ExportEngineService.exportNetwork henüz implemente edilmedi (bkz. not).
    // Şimdilik burada eklenen stub çağrılıyor; gerçek export mantığı
    // (CSV/GraphML/resim vb.) ExportEngineService içine yazılmalı.
    (exportEngine as any).exportNetwork?.(rawNetwork, { format });
  };
