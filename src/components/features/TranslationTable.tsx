...
export const STANDARD_LANGS = [
  { key: "TÃ¼rkÃ§e", title: "TÃ¼rkÃ§e", color: "text-gray-900 dark:text-white" },
  { key: "BatÄ± AdÄ±gece", title: "BatÄ± AdÄ±gece", color: "text-emerald-600 dark:text-emerald-400 font-semibold" },
  { key: "DoÄŸu Kabardeyce", title: "DoÄŸu Kabardeyce", color: "text-blue-600 dark:text-blue-400 font-semibold" },
  { key: "Ä°ngilizce", title: "Ä°ngilizce", color: "text-indigo-600 dark:text-indigo-400" },
  { key: "RusÃ§a", title: "RusÃ§a", color: "text-amber-600 dark:text-amber-400" },
  { key: "ArapÃ§a", title: "ArapÃ§a", color: "text-teal-600 dark:text-teal-400" },
] as const;

export interface TranslationRow {
  kavram?: string;
  detaylar?: Array<{ kaynak_sozluk?: string; tanim: string; dialect?: string }>;
  [key: string]: any; // Dillerin dinamiÄŸini destekler (TÃ¼rkÃ§e: "kapÄ±", Ä°ngilizce: "door" vb.)
}
...
export function TranslationTable({ rows }: TranslationTableProps) {
  // Akordeon / Detay gÃ¶sterimi iÃ§in seÃ§ilen satÄ±rÄ±n indeksi
  ...
  return (
    <div className="w-full space-y-4">
      {/* ğŸ“± MOBÄ°L GÃ–RÃœNÃœM (md boyutundan kÃ¼Ã§Ã¼k ekranlarda Kart ÅŸeklinde basar) */}
      <div className="block md:hidden space-y-3">
        {rows.map((row, index) => {
          const mainConcept = row.kavram || row["TÃ¼rkÃ§e"] || row["BatÄ± AdÄ±gece"] || row["DoÄŸu Kabardeyce"] || "Kavram";
            ...
            >
              ...
              <div className="flex justify-between items-center border-b pb-2 border-gray-100 dark:border-gray-800">
                ...
                {row.detaylar && row.detaylar.length > 0 && (
                  ...
                  >
                    {isExpanded ? "Kapat â–²" : `TanÄ±mlar (${row.detaylar.length}) â–¼`}
                  </button>
                )}
              </div>
              ...
              {/* TÄ±klayÄ±nca AÃ§Ä±lan Orijinal SÃ¶zlÃ¼k TanÄ±mlarÄ± */}
              {isExpanded && row.detaylar && (
                <div className="pt-2 border-t border-gray-100 dark:border-gray-800 space-y-2">
                  <div className="text-xs font-semibold text-gray-500">Kaynak SÃ¶zlÃ¼kler & Detaylar:</div>
                  {row.detaylar.map((detay, dIdx) => (
                    <div key={dIdx} className="p-2.5 rounded bg-gray-50 dark:bg-gray-950 text-xs space-y-1">
                      <div className="flex justify-between text-[11px] font-medium text-gray-400">
                        <span>ğŸ“– {detay.kaynak_sozluk || "Bilinmeyen SÃ¶zlÃ¼k"}</span>
      ...
      {/* ğŸ–¥ï¸ MASAÃœSTÃœ GÃ–RÃœNÃœM (md ve Ã¼zeri ekranlarda Tablo olarak basar) */}
        ...
        <table className="w-full text-sm text-left border-collapse">
          <thead className="bg-gray-50 dark:bg-gray-800/80 text-gray-600 dark:text-gray-300 text-xs uppercase font-semibold border-b border-gray-200 dark:border-gray-800">
            <tr>
              ...
              <th className="px-4 py-3.5 text-right">AyrÄ±ntÄ±</th>
            </tr>
          </thead>
            ...
            {rows.map((row, index) => {
              ...
              return (
                <React.Fragment key={index}>
                  <tr className="hover:bg-indigo-50/30 dark:hover:bg-indigo-950/20 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-right">
                      {row.detaylar && row.detaylar.length > 0 ? (
                        ...
                        >
                          {isExpanded ? "Kapat â–²" : `Detay (${row.detaylar.length}) â–¼`}
                        </button>
                      ) : (
                  ...
                  {/* MasaÃ¼stÃ¼ SatÄ±r Ä°Ã§i AÃ§Ä±lÄ±r Panel (Accordion Detail) */}
                  {isExpanded && row.detaylar && (
                      ...
                      <td colSpan={STANDARD_LANGS.length + 1} className="p-4">
                        <div className="space-y-3">
                          <h5 className="font-semibold text-xs text-indigo-900 dark:text-indigo-300 uppercase tracking-wider">
                            Orijinal Kaynaklar ve TanÄ±mlar
                          </h5>
                            ...
                            {row.detaylar.map((detay, dIdx) => (
                              ...
                              >
                                <div className="flex justify-between items-center text-[11px] font-medium text-gray-400">
                                  <span>ğŸ“– {detay.kaynak_sozluk || "SÃ¶zlÃ¼k"}</span>
                                  {detay.dialect && (
