...
export const hedefDilBul = (dosyaAdi?: unknown): string => {
  ...
  if (isim.includes("tur") || isim.includes("tu-")) {
    return "tr";
  }

  if (isim.includes("ara") || isim.includes("-ar")) {
    return "ar";
  }

  if (isim.includes("en") || isim.includes("kbd-en")) {
    return "en";
  }

  if (isim.includes("rus") || isim.includes("ru-")) {
    return "ru";
  }
...
export const tanimlariBicimlendir = (
  tanim: unknown,
  tema: TemaTipi = VARSAYILAN_TEMA as TemaTipi,
  gecerliBaslikOrBoyut?: string | number,
  metinBoyutuParam?: number,
  kaynakParam?: unknown,
  sozluklerListesi?: unknown[],
) => {
  ...
  const temizBaslik = gecerliBaslik
    ? normalizeText(gecerliBaslik)
    : "";
  ...
  let turBilgisi = "";
  let kaynakBilgisi = kaynagiDuzenle(
    kaynakParam,
    sozluklerListesi,
  );
  ...
  for (const satir of satirListesi) {
    ...
    if (typeMatch) {
      const tur = typeMatch[1].trim().toLowerCase();
      turBilgisi = TUR_MAP[tur] || typeMatch[1].trim();
      continue;
    }

    const sourceMatch = temiz.match(
      /^(?:source|kaynak):\s*(.*)$/i,
    );

    if (sourceMatch) {
      kaynakBilgisi = kaynagiDuzenle(
        sourceMatch[1].trim(),
        sozluklerListesi,
      );
      continue;
    ...
    if (
      temizBaslik &&
      normalizeText(temiz) === temizBaslik
    ) {
      continue;
    }
      ...
      {anlamlar.length > 0 && (
        <div>
          ...
          {anlamlar.map((anlam, idx) => {
            const isRomaRakami =
              /^(I|II|III|IV|V|VI|VII|VIII|IX|X)(\.|\s|$)/i.test(
                anlam,
              );

            const sayiMatch = anlam.match(
              /^(\d+)[\.\)]\s*(.*)$/,
            );
            ...
            if (isSayiBasligi && sayiMatch) {
              const romaRakami = sayiyiRomaRakaminaCevir(
                parseInt(sayiMatch[1], 10),
              );

              const kalanMetin = sayiMatch[2];

              gosterilecekMetin = kalanMetin
                ? `${romaRakami}. ${kalanMetin}`
                : romaRakami;
            }
      ...
      {Boolean(kaynakBilgisi) && (
        <div
          style={{
            ...
            borderTop: turBilgisi
              ? "none"
              : `1px solid ${gecerliTema.kenarlik}`,
