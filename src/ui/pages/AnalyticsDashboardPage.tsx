...
export const AnalyticsDashboardPage: React.FC<AnalyticsDashboardPageProps> = ({
  ...
  if (!analyticsData) {
    ...
    }, 'Analiz verisi yÃ¼kleniyor veya bulunamadÄ±.');
  }

  return React.createElement('div', { className: 'p-6 space-y-6 bg-gray-50 min-h-screen' },
    React.createElement('h1', { className: 'text-2xl font-bold text-gray-800 border-b pb-3' }, 'ğŸ“Š Knowledge Graph Analytics Dashboard'),
    
    React.createElement('div', { className: 'grid grid-cols-1 md:grid-cols-3 gap-6' },
      React.createElement('div', { className: 'bg-white p-5 rounded-lg border shadow-sm', 'data-testid': 'metric-coverage' },
        React.createElement('div', { className: 'text-sm font-semibold text-gray-500' }, 'Diyalekt Kapsama OranÄ± (Scov)'),
